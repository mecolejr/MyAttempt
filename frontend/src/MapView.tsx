...
export default function MapView({ valuesDiversity, onSelectLocation }: { valuesDiversity: boolean; onSelectLocation?: (p: { name: string; state: string }) => void }) {
  ...
  return (
    <div style={{ height: 360, marginTop: 24, position: 'relative' }}>
      ...
      {stateResetControl({ onReset: () => {
        // Provide an optional hook to reset state filter via parent, if used
        if (onSelectLocation) onSelectLocation({ name: '', state: '' } as any)
      }})}
      {loading && (
        <div style={{
          position: 'absolute',
...
function Legend() {
  ...
  useEffect(() => {
    ...
    control.onAdd = () => {
      ...
      div.innerHTML = `
        <div style="background:var(--tp-bg);border:1px solid var(--tp-border);border-radius:8px;padding:8px 10px;font:12px/1.3 system-ui,sans-serif;box-shadow:0 2px 8px rgba(0,0,0,.1)">
...
function stateResetControl({ onReset }: { onReset: () => void }) {
  return (
    <button
      onClick={onReset}
      className="btn"
